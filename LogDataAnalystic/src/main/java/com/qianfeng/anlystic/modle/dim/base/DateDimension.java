...
public class DateDimension extends BaseDimension{
    ...
    public static DateDimension buildDate(long time,DateEnum type){
        ...
        calendar.clear(); //清空日历对象
        if(DateEnum.YEAR.equals(type)){
            calendar.set(year,0,1); //如果是年指标，默认在该年的1.1号执行
            return new DateDimension(year,0,0,0,
                    1,type.typeName,calendar.getTime());
        }
        int season = TimeUtil.getDateInfo(time,DateEnum.SEASON); //如果季度指标，则是当前季度的第一个月
        ...
        int month = TimeUtil.getDateInfo(time,DateEnum.MONTH); //如果月指标，则是当前月的第一天
